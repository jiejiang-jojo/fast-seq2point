...
class DataGenerator(object):
    ...
    def generate_validate(self, data_type, max_iteration, shuffle=True):
        ...
        iteration = 0
        pointer = 0
        steps = self.width * self.batch_size
        ...
        while pointer < len(indexes):
            ...
            if shuffle:
                ...
                # the first (seq_len // 2) elements in x is ignored
                # in the testing
                if pointer + self.seq_len + steps - 1 > len(indexes):
                    break
                batch_x = x[pointer : pointer + self.seq_len + steps - 1]
                batch_y = y[pointer + self.seq_len // 2 : pointer + self.seq_len // 2 + steps]
